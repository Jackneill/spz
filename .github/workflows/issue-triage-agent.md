...
safe-outputs:
  add-labels:
    allowed: [wontfix, question, invalid, good-first-issue, ci, feat, duplicate, doc, deps, bug, binding-python, binding-c]
  add-comment: {}
imports:
  - shared/reporting.md
---
...
List open issues in ${{ github.repository }} that have no labels. For each unlabeled issue, analyze the title and body, then add one of the allowed labels: `wontfix`, `question`, `invalid`, `good-first-issue`, `ci`, `feat`, `duplicate`, `doc`, `deps`, `bug`, `binding-python`, `binding-c`.
